...
/**
 * Tất cả function bên dưới sẽ chỉ request và lấy data luôn, mà không có try catch hay then catch gì để bắt lỗi
...
export const updateBoardDetailsAPI = async (boardId, updateData) => {
  const res = await authorizeAxiosInstance.put(
    `${API_ROOT}/v1/boards/${boardId}`, updateData)
  return res.data
}

export const moveCardToDifferentColumnAPI = async (updateData) => {
  const res = await authorizeAxiosInstance.put(`${API_ROOT}/v1/boards/supports/moving_card`, updateData)
  return res.data
}
...
export const updateCardDetailsAPI = async (cardId, updateData) => {
  const res = await authorizeAxiosInstance.put(`${API_ROOT}/v1/cards/${cardId}`, updateData)
  return res.data
}
...
export const registerUserAPI = async (data) => {
  ...
  toast.success('Account created successfully! You can now log in.', { theme: 'colored' })
  return res.data
}
...
export const inviteUserToBoardAPI = async (data) => {
  const res = await authorizeAxiosInstance.post(`${API_ROOT}/v1/invitations/board`, data)
